<?php
class ControllerCommonChannelout extends Controller {
	public function index() {
		$this->load->language('common/channelout');
		...
		foreach($channels as $channel){ 
		    ...
		    if($set_demod_resp == false || $set_demod_decoded['error'] == true){
				...
				$data['datachannels'][$i]=array('channelnumber'=>$channel['channelnumber'],'output_channelname'=>$channel['output_channelname'],'Qam_Freq'=>$channel['Qam_Freq'],'rmx_no'=>$channel['rmx_no'],'input'=>$channel['inputid'],'output'=>$channel['targetid'],'changed_ch_number'=>$channel['changed_ch_number'],'changed_ch_name'=>$channel['changed_ch_name'],'bitrate'=>$set_demod_decoded['Band'][1]);
				$i++;
		...
		$this->response->setOutput($this->load->view('common/channelout.tpl', $data));
	}
}