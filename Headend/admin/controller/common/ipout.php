<?php
class ControllerCommonIpout extends Controller {
	public function index() {
		$this->load->language('common/ipout');
		...
		$data['ipdata']=array();
		$i=0;
		$ips= $this->model_common_dashboardchannels->getIpData();
		...
		foreach($ips as $ip){ 
		 ...
		 //    if($set_demod_resp == false || $set_demod_decoded['error'] == true){
			...
			// }else{
				// $this->response->setOutput($set_demod_resp);ip_address,port,payload_interface
				$data['ipdata'][$i]=array('ip_address'=>$ip['ip_address'],'port'=>$ip['port'],'payload_interface'=>$ip['payload_interface'],'signalstrength'=>0);
				$i++;
		...
		$data['channels']= $this->model_common_dashboardchannels->getoutputChannelwithFrequency();
		...
		$this->response->setOutput($this->load->view('common/ipout.tpl', $data));
	}
}